...
app.post("/auth/login", (req, res) => {
  ...
  if (user !== config.adminUser || password !== config.adminPassword) return res.status(401).json({ error: "invalid credentials" });
...
app.post("/orders/reserve", async (req, res) => {
  try {
    const { phone, name, items } = req.body as { phone: string; name?: string; items: { productId: string; quantity: number }[] };
    if (!phone || !items?.length) return res.status(400).json({ error: "phone y items requeridos" });
...
app.post("/orders/:id/status", auth, async (req, res) => {
  ...
  const { status } = req.body as { status: "RESERVADO" | "EN_PROCESO" | "TERMINADO" };
...
app.post("/payments/yape/webhook", async (req, res) => {
  const sig = (req.headers["x-yape-signature"] as string) || "";
  if (config.yapeWebhookSecret) {
    const expected = crypto.createHmac("sha256", config.yapeWebhookSecret).update((req as any).rawBody || "").digest("hex");
    if (expected !== sig) return res.status(401).json({ error: "firma inválida" });
  }
  const { orderId, paymentRef } = req.body as { orderId: string; paymentRef?: string };
