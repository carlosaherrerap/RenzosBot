...
export async function sendImageToPhone(phone, image, caption) {
    const jid = `${phone}@s.whatsapp.net`;
    if (!sockRef)
        return;
        ...
        if (msg.message.imageMessage) {
            ...
            try {
                ...
                if (items.length > 0) {
                    ...
                    const otros = items.slice(1).map((i) => i.name).join(", ");
...
async function synthAudio(text) {
    ...
    const b64 = r.data.audio_base64;
    return Buffer.from(b64, "base64");
}
async function respondWithMode(jid, mode, text) {
    if (mode === "audio") {
